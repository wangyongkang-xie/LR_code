...
def clean_test(helper,
    ...
    main.logger.info('___Test-clean {} poisoned: {}, Average loss: {:.4f}, '
                     'Accuracy: {}/{} ({:.4f}%)'.format(model.name, is_poison,
...
def adv_test(helper,
    ...
    if helper.params['type'] == config.TYPE_LOAN:
        ...
        data_iterator = helper.test_data_poison #no target indx
        with torch.no_grad():
            for batch_id, batch in enumerate(data_iterator):
    ...
    main.logger.info('___Test-poison {}, poisoned: {}, Average loss: {:.4f}, '
                     'Accuracy: {}/{} ({:.4f}%)'.format(model.name, is_poison,
